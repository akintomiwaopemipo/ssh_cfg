...
pub enum SshOptionKey {
    ...
    ///
    /// See `SendEnv` and `SetEnv` in ssh_config(5) for how to configure the
    /// client. The `TERM` environment variable is always accepted whenever the
    ...
    /// characters `*` and `?`. Multiple environment variables may be separated
    ...
    /// permitted.
    ///
    /// The available options are `yes` (the default) or `all` to allow
    /// `StreamLocal` forwarding, `no` to prevent all `StreamLocal` forwarding,
    /// `local` to allow local (from the perspective of ssh(1)) forwarding only
    /// or `remote` to allow remote forwarding only. Note that disabling
    ...
    ///
    /// The available options are yes (the default) or all to allow TCP
    /// forwarding, no to prevent all TCP forwarding, local to allow local (from
    /// the perspective of ssh(1)) forwarding only or remote to allow remote
    ...
    /// authentication method names, or by the single string any to indicate the
    ...
    /// device identifier bsdauth or pam. depending on the server configuration.
    /// For example, "keyboard-interactive:bsdauth" would restrict keyboard
    /// interactive authentication to the bsdauth device.
    ///
    /// If the publickey method is listed more than once, sshd(8) verifies that
    /// keys that have been used successfully are not reused for subsequent
    /// authentications. For example, "publickey,publickey" requires successful
    ...
    /// password-less accounts when PermitEmptyPasswords is enabled), "password"
    /// and "publickey".
    ...
    /// sshd(8). Arguments to `AuthorizedKeysFile` accept the tokens described
    /// in the TOKENS section. After expansion, `AuthorizedKeysFile` is taken to
    ...
    /// option may be set to `none` to skip checking for user keys in files. The
    ...
    /// `AuthorizedPrincipalsCommand` accept the tokens described in the TOKENS
    ...
    /// `AuthorizedPrincipalsCommandUser` is not, then sshd(8) will refuse to
    /// start.
    ...
    ///
    /// Arguments to `AuthorizedPrincipalsFile` accept the tokens described in
    /// the TOKENS section. After expansion, `AuthorizedPrincipalsFile` is taken
    ...
    ///
    /// If the argument is none then no banner is displayed. By default, no
    /// banner is displayed.
    Banner,
    ...
    /// authentication.
    ///
    /// At session startup sshd(8) checks that all components of the pathname
    /// are root-owned directories which are not writable by any other user or
    /// group. After the chroot, sshd(8) changes the working directory to the
    /// user's home directory. Arguments to ChrootDirectory accept the tokens
    ...
    /// sessions which use logging may require `/dev/log` inside the chroot
    /// directory on some operating systems (see sftp-server(8) for details).
    ///
    /// For safety, it is very important that the directory hierarchy be
    /// prevented from modification by other processes on the system (especially
    /// those outside the jail). Misconfiguration can lead to unsafe
    ...
    /// different from TCPKeepAlive. The client alive messages are sent through
    ...
    /// by spaces.
    ///
    /// Login is disallowed for users whose primary group or supplementary group
    /// list matches one of the patterns. Only group names are valid; a
    ...
    /// client is available in the `SSH_ORIGINAL_COMMAND` environment variable.
    ...
    /// acceptor a client authenticates against.
    ///
    /// If set to `yes` then the client must authenticate against the host
    /// service on the current hostname. If set to `no` then the client may
    ...
    /// replacing them. If the specified value begins with a `-` character, then
    ...
    /// specified by `HostKey`. The default behaviour of sshd(8) is not to load
    /// any certificates.
    HostCertificate,
    ...
    /// group/world-accessible and that the `HostKeyAlgorithms` option restricts
    ...
    /// has access to the private host keys.
    ///
    /// If the string
    ...
    /// during `HostbasedAuthentication` and use only the system-wide known
    ...
    /// `ListenAddress` options are permitted. For more information on routing
    ...
    /// logged in.
    ///
    /// If the value is 0, there is no time limit. The default is
    /// 120 seconds.
    LoginGraceTime,
    ...
    /// multiplexing. Setting MaxSessions to 1 will effectively disable session
    ...
    /// described in the `PATTERNS` section in ssh_config(5). The wildcard `*`
    ...
    /// all forwarding requests. The wildcard `*` can be used for host or
    ...
    ///
    /// The argument must be `yes`, `prohibit-password`, `forced-commands-only`,
    ...
    /// in interactively.
    ///
    /// (On some systems it is also printed by the shell, `/etc/profile`, or
    ...
    /// will be bound to this rdomain(4). If the routing domain is set to %D,
    /// then the domain in which the incoming connection was received will be
    /// applied.
    RDomain,
    ...
    /// Note that if this file is not readable, then public key authentication
    /// will be refused for all users. Keys may be specified as a text file,
    /// listing one public key per line, or as an OpenSSH Key Revocation List
    /// (KRL) as generated by ssh-keygen(1). For more information on KRLs, see
    ...
    /// their directory or files world-writable. The default is `yes`. Note that
    ...
    /// permitted for authentication using `TrustedUserCAKeys`. For more details
    ...
    /// Specifies whether sshd(8) attempts to send authentication success and
    /// failure messages to the blacklistd(8) daemon.
    ///
    /// The default is `no`. For forward compatibility with an upcoming
    /// blacklisted rename, the `UseBlocklist` alias can be used instead.
    UseBlacklist,
    ...
    ///
    /// If this option is set to `no`, then only addresses and not host names
    /// may be used in `~/.ssh/authorized_keys` from and `sshd_config` Match
    ...
    /// banner sent by the server upon connection.
    ///
    /// The default is `FreeBSD-20200214`. The value `none` may be used to
    /// disable this.
    VersionAddendum,
    ...
    /// to listen on the wildcard address (see `X11UseLocalhost`), though this
    ...
    /// loopback address or to the wildcard address.
    ///
